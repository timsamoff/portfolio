...
|---|---|---|---|---|
| Category registry parity | `categories.json` changed | `app.js`'s `filterAliases` literal NOT changed in the same commit | "Did you add a new category with a shortcut? If so, add the same key/value to `filterAliases` in `app.js` (~line 812)." | **Implemented** (blocking) — `.githooks/lib/pre-commit-checks.js` `checkCategoryRegistryParity` |
| Demo category parity | `categories.json` changed | `demo/demo-data.js`'s `DEMO_CATEGORIES` NOT changed in the same commit | "Demo doesn't read `categories.json` — does `DEMO_CATEGORIES` in `demo-data.js` (line 6) need the same category added?" | **Implemented** (blocking) — `checkDemoCategoryParity` |
