...
| Rule | Trigger (files changed) | Flag condition | Required fix / self-review question | Status |
|---|---|---|---|---|
| Category registry parity | ~~`categories.json` changed~~ | ~~`app.js`'s `filterAliases` literal NOT changed in the same commit~~ | — | **Retired** — `app.js` no longer hardcodes `filterAliases`; it's derived dynamically from `categories.json` at runtime (`buildFilterAliases()`), so no code change is ever needed there. The check's premise no longer applies; removed from both `pre-commit-checks.js` and `repo-wide-scan.js` rather than left to false-positive forever. |
| Demo category parity | `categories.json` changed | `demo/demo-data.js`'s `DEMO_CATEGORIES` NOT changed in the same commit | "Demo doesn't read `categories.json` — does `DEMO_CATEGORIES` in `demo-data.js` (line 6) need the same category added?" | **Implemented** (blocking) — `checkDemoCategoryParity`. Confirmed intentional by Tim; suppressed via a `gate-exceptions.json` entry (category: "Demo category parity", file_scope: "demo/demo-data.js", review_by 2027-09-10). |
| Category-migration shim sync | Any of `app.js` / `demo/demo-app.js` / `demo/demo-data.js` changed near a `project.category` / `.categories` reference | Only one of the three files touched in the commit | "This is one of three independent copies of the legacy-category migration shim (app.js ~1310, demo-app.js ~1061/1172/1212/1424, demo-data.js ~294/354). Does the same fix apply to the other two?" | **Deferred** — the three call sites don't share a single grep-able function name (it's an inline `if` pattern repeated, not a named function), so a reliable "was this specific shim touched" signal isn't mechanically distinguishable from any other edit to those files without much higher false-positive risk than the other rules. The general-purpose Demo/main parity check (function-name diffing) provides partial, coarser coverage for `app.js`/`demo-app.js` edits. |
| CSS custom property existence | Any `.css`/`.html`/`.js` file adds a new `var(--...)` reference | The referenced token name has no matching definition found via grep in `style.css`'s `:root` block | "`var(--x)` is referenced in <file> but not defined in `style.css`'s `:root`. Either define it there or fix the typo (see the `--text-muted`/`--color-text-muted` bug, 7 files/31 occurrences, for exactly this failure mode)." | **Implemented** (blocking) — `checkCssCustomPropertyExistence`, live-tested against a deliberately undefined token |
| Demo/main parity | `app.js` or `admin.js` changed | Matching function name (by grep on function name) not touched in `demo/demo-app.js` / `demo/demo-admin.js` in the same commit | "Per CLAUDE.md working agreement, `demo/` must mirror behavioral changes to `./`. Is this change one of the documented exceptions (localStorage, demo media, demo-only UI)? If not, mirror it into demo, or add a `gate-exceptions.json` entry (see Part 5) explaining why not." | **Implemented** (warning, non-blocking by design — judgment calls apply per the original spec) — `checkDemoMainParity`, walks diff hunk context to attribute body-level edits to their enclosing function, not just signature-line edits |
| Shared-style leakage | `demo-override.css` or an admin-only stylesheet changed | The changed rule sets a color/spacing/type-scale value already defined in `style.css` `:root` as a token, rather than referencing the token | "Per CLAUDE.md, shared values belong only in `style.css`. Does this new rule duplicate a value that already has a token? Reference the token instead of a literal." | **Implemented** (warning, non-blocking — heuristic literal-value grep, not semantic) — `checkSharedStyleLeakage` |
| `cleanupMalformedLinks` triplication | Any of `admin.js` / `demo/demo-admin.js` / `utilities/fix-links.js` changed near `cleanupMalformedLinks` | Only one of the three files touched | "This regex chain is triplicated. Does the fix apply to the other two copies?" | **Implemented** (warning, non-blocking) — `checkCleanupMalformedLinksSync` |
| Share-URL signature match | `generateShareUrl` changed in `app.js` or `demo/demo-app.js` | The function signature (param count/names) differs between the two after the change | "`generateShareUrl` signatures have already drifted once (demo has an extra `mediaArray` param main doesn't). Confirm this change doesn't widen that gap, given two of the last five commits were unresolved share-bug fixes." | **Implemented** (warning, non-blocking) — `checkShareUrlSignatureMatch`. The drift this was flagging is fixed: demo's `mediaArray` parameter was dead code (never read in the function body) — removed from the signature and all 3 call sites. Signatures now match. |
| README currency | `projects.json` schema, `categories.json` schema, or any documented feature (multi-category, shortcuts, `<award>`/`<live>` tags, demo mode) changed | README.md NOT changed in the same commit | "Does this change alter anything README.md's Data Format, Categories, Description Formatting, or Demo Mode sections describe? If yes, update the matching section (see Part 2's proposed structure)." | **Implemented** (blocking) — `checkReadmeCurrency`, gated on a marker-word heuristic for app.js/admin.js to avoid false positives on unrelated edits |
| Commit message conventions | Every commit | Message is a sprawling multi-paragraph narrative, or contains a co-author line | "Keep to a single-line summary (or a few one-sentence bullets if genuinely needed). No co-author notes. One commit per session unless the change set is large enough to split." | **Implemented** as a `commit-msg` hook (blocking for subject-length/narrative-body violations; attribution lines are mechanically stripped rather than just flagged) — `.githooks/lib/commit-msg-checks.js` |
| Exception check | Any flagged violation above | A matching, unexpired entry exists in `gate-exceptions.json` scoped to this exact file | If found, suppress the flag and print the exception's reason/owner/review-by date instead of blocking. If the entry's review-by date has passed, treat it as if no exception exists — flag anyway. | **Implemented** — `.githooks/lib/gate-exceptions.js`, live-tested for both an active and an expired exception; file is absent from the repo by design (no entries pre-populated) |

Implemented in `.githooks/` (versioned, enabled via `git config core.hooksPath .githooks`), wired via `.githooks/pre-commit` and `.githooks/commit-msg` shims calling into `.githooks/lib/*.js` (Node, no external dependencies). See `production_notes/todo.md` for the pre-existing-violations backlog this surfaced and the disposition of each original candidate.
...
**Implementation status:** the checker (`.githooks/lib/gate-exceptions.js`) is implemented and live-tested (active exception suppresses a flag; expired exception is treated as no exception). `gate-exceptions.json` itself was deliberately NOT created — its absence is handled gracefully (treated as zero exceptions) rather than erroring. Tim can create the file himself when/if he decides either of the proposals below (or any other) reflects genuine intent.
