...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openMediaModal(mediaArray, startIndex = 0, projectId = null) {
        ...
        let projectTitle = '';
        if (projectId !== null && window.__projectsData && window.__projectsData[projectId]) {
            projectTitle = window.__projectsData[projectId].title || '';
        }

        let slidesHtml = '';
        mediaArray.forEach((mediaUrl, idx) => {
            const mediaHtml = generateModalMediaHtml(mediaUrl, idx, projectTitle);
    ...
    function loadProjects(projects) {
        ...
        visibleProjects.forEach((project, visibleIdx) => {
            ...
            article.innerHTML = `
                ...
                <div class="item-content">
                    <span class="category-tag" data-category-filter="${escapeHtml(categories.join(','))}">${escapeHtml(displayCategories)}</span>
        ...
        if (filterNav) {
            ...
            const hasUncategorizedProjects = visibleProjects.some(p => {
                let cats = [];
                if (p.categories && Array.isArray(p.categories)) {
                    cats = p.categories;
                } else if (p.category) {
    ...
    function scrollToFilterPills() {
        ...
        const targetScrollY = filterTop - headerH - getFilterNavTopGap(filterNav);
